  ...
  for (let round = 0; round < 8; round++) {
    ...
    for (const track of scene.timeline.tracks) {
      ...
      for (let i = 0; i < track.segments.length; i++) {
         ...
         * travel exists only because a POSSESSION was interrupted at its moment. The check is
         * STRUCTURAL, not clocked: a possession must feed the roll — either a possessed segment
         * before it, or the initial holder when the roll is the ball's first movement. (A step-1
         * roll fires at t=0, where the compiled clock has already handed the ball to the roll
         * itself, so asking `stateAt` would kill every legitimate first-step roll.)
         */
        if (seg.implicit) {
          const before = track.segments.slice(0, i)
          const fed =
            before.some((s) => s.kind === 'possessed') ||
            (!!draft.ball.initialHolderId && !before.some((s) => 'path' in s))
          if (!fed) {
