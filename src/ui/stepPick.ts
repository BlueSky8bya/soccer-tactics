 ...
 *  - the board stands at the step's opening, but only under isolation: 전체 보기 draws the whole
 *    play from kickoff, and there is no single frame a step means there.
 *
...
export function pickStep(doc: TacticDocument, compiled: CompiledTimeline, n: number): void {
  ...
  st.setCurrentStep(n)
  if (st.stepIsolate) st.setPlayhead(stepOpensAt(doc, compiled, n))
}
