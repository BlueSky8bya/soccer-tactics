...
Last Updated: 2026-08-24 (세션 21, PLAN-015 — 단계 격리·단계 상황판·공 던지기 토글·다크모드) (직전: 세션 20, PLAN-014 핵심 재현 무결성 감사)
Project Version: 0.1.0
Harness Protocol: project-initializing_260712.md (schema 1.1) — `agent-harness.yaml`

## Current Objective

**보여주는 것을 줄이고, 무엇을 하는지 말하게 한다 (2026-08-24, PLAN-015 완료).**
사용자 요청 4건: 공 굴러가는 토글(기본 꺼짐), 단계를 고르면 "지금 상황 + 이번에 할 일"이 보이게,
처음부터 끝까지 다 보여주지 말 것, 다크모드.

- **단계 격리**(기본 켜짐): 경로·배지는 `현재`만 선명, `현재-1`은 trace, 나머지는 **미렌더**.
  잔상은 다른 규칙 — 엔티티별 **현재 직전의 마지막 잔상**이 trace로 남는다(3단계 전에 움직인 선수의
  현재 위치가 사라지지 않게). 숨긴 것은 pick 입력에서도 빠진다. StepBar "보기: 이 단계 / 전체"가 탈출구.
- **단계 상황판**: 보드 좌상단이 `N단계 · 지금 … · 이번 …`. 상황은 **직전 단계들이 끝난 상태**에서
  읽는다(경계에서 도착과 출발이 겹쳐 ε 방향에 따라 한쪽이 틀리기 때문). 재생 중엔 진행 중인 단계.
- **공 휙 던지기**: `uiStore.ballFling` 기본 **꺼짐**. 상수(ISSUE-008)는 손대지 않고 게이트만 추가.
  더블클릭 슬링샷은 명시적 제스처이므로 유지.
...
PLAN-015는 구현·검증 완료다. 남은 것은 사람이 봐야 하는 것뿐이다.

1. (사용자) `npm run dev` → 이번에 바뀐 것 체감:
   - 단계 칩을 2, 3으로 옮겨가며 **보이는 양이 줄어드는 게 도움이 되는지**, 아니면 직전 1단계보다
     더(또는 덜) 남아야 하는지. 남는 범위는 `deriveStepLayers` 한 곳에서 바꾼다.
   - 좌상단 **상황판 문구**가 실제로 읽히는지 — 특히 "이번"에 나열되는 행동 이름
     (`이동 / 드리블 / A→B 패스 / 슛 / 공간으로`)이 맞는 말인지.
   - **공 휙 던지기**가 꺼진 기본 상태가 편한지. 켜면 종전과 동일하다.
   - 다크모드에서 대비가 부족한 곳(특히 잔디 위 경로·잔상, tint 버튼).
2. (사용자) 남은 체감 항목: ISSUE-004 spring/pulse 강도, ISSUE-007 자동대응 품질, ISSUE-008 fling 상수
   (플링은 이제 옵트인이라 **기본 사용자에게는 노출되지 않는다** — 상수 조정 우선순위가 내려간다).
3. (선택) E-polish — contrast/CLS/광범위 viewport. 다크 테마가 생겼으므로 대비 검사는 **두 테마 모두**
   대상이다.
