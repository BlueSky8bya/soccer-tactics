...
Last Updated: 2026-08-26 (세션 23, ADR-0009 v41 — 단축키 캡 활자) (직전: 세션 22, PLAN-017 전면 캔버스)
...
- **팀 구성 ▾** 상단바 팝오버(포메이션 + 양 팀 채우기). **정리·동작 설정은 오른쪽 아래 열**
  (아이콘+캡, v36) — 왼쪽 열에 쌓았더니 펼칠 때 보드를 129px 넘었다. **활자는 열 폭을 따른다**
  (v41): 캡 하한 11.5px(맞은편 `guideCap`과 동일)·`--st-text-2`, 낱말이 나오면 13.5/12,
  210px을 넘으면 15/13에 행 42px.
