...
**보여주는 것을 줄이고, 그림이 말 못 하는 것만 말한다 (2026-08-24, PLAN-015 v2 완료).**
...
- **경로 방향 언어 통일**: 선수·공 경로 모두 점선 + 목적지 쪽으로 **행진하는 대시**(같은 속도),
  ...
  틈이 흰색으로 메워지지 않는다.
- **단계 시계**(보드 우상단): `N단계 · {d}초 걸림 · {i}/{n}번째 · {from}초에 시작 · 전체 {all}초`.
  이전의 "누가 보유·누가 패스" 서술은 그림이 이미 말하는 것이라 폐기(사용자 지적).
- **공 휙 던지기**: 헤더 1행 스위치, 기본 **꺼짐**. 상수(ISSUE-008)는 그대로.
- **다크모드**: 헤더 버튼 시스템 → 라이트 → 다크 순환, localStorage, React 마운트 전 도색.

증거: `node pw/run.cjs` **138 checks ALL PASS**. 상세: CHG-20260824-194·195·196.
...
PLAN-015 v2는 구현·검증 완료다. 남은 것은 사람이 봐야 하는 것과 미재현 버그 1건이다.

1. **(미해결) 사용자 보고 — "같은 선수한테 또 공을 2번 이상 주면 반응을 안 해".**
   현재까지 재현 실패. 프로브로 확인한 것: 드롭으로 같은 선수에게 두 번 주기(정상, 토스트 발화),
   같은 선수가 두 번 수신하는 패스 체인 A→B→C→B(정상, 4단계까지 생성), B→B 자기 패스(생성됨).
   재현된 유일한 "아무 일 없어 보이는" 경로는 **공을 시작 지점에서 다시 집어 같은 선수에게 패스**할
   때다 — 그 제스처는 정의상 이후 공 체인을 지우고 같은 패스를 다시 만들므로 화면이 그대로다
   (토스트만 "이후 패스 N개는 지워졌어요"). 사용자에게 정확한 조작 순서를 물어야 한다.
2. (사용자) `npm run dev` → 체감 확인:
   - 단계 칩을 옮길 때 **보드가 그 단계 시작 프레임으로 서는 것**이 의도대로인지.
   - 행진하는 대시의 **속도(26 units/s)와 리듬**(런 11/5, 패스 6/5)이 읽기 좋은지.
   - 우상단 **단계 시계** 문구가 실제로 쓸모 있는지(더 필요한 수치가 있는지).
   - 다크모드 대비.
3. (선택) E-polish — contrast/CLS/광범위 viewport. 다크 테마가 생겼으므로 **두 테마 모두** 대상.
4. (선택) 포인터 마라톤 재작성 — 과거 `pw/marathon.cjs`는 소스가 없다.
...
- `npm test` → PASS (49 files / **345 tests**) — 2026-08-24
- `npm run build` → PASS — 2026-08-24
- `npm run harness:verify` → PASS (0 warnings) — 2026-08-24
- 브라우저 probe `node pw/run.cjs` (7종) → **138 checks ALL PASS**, 콘솔 클린 — 2026-08-24
