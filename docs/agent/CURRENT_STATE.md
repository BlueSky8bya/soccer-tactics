...
- **키 가이드**(`KeyGuide`, v33): **왼쪽 여백**(피치가 높이 제약이라 영원히 남는 잔디)의 200px 열.
  만들기/보기/정리 3그룹, 행마다 `[캡] 낱말`. 키를 실제로 쥐면 그 행만 열리며 강조되고(ExposeHK
  리허설), 호버·포커스로도 열리고 클릭은 고정한다. 상세는 **열 안에서만** 열려 피치를 덮지 않는다.
...
5게이트 PASS(353 tests), 브라우저 209 checks ALL PASS(`pw/full-bleed` 30 — 레이아웃·발견성 계약 상시 감시).
