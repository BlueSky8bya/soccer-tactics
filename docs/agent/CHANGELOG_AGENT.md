...
## CHG-20260825-211 — 인터페이스 재단장: 하나의 형태 사다리 · 하나의 누름 · 바뀐 것을 말하는 보드

Date: 2026-08-25 · Type: UX · Level: L2 (PLAN-20260825-016)

Problem (사용자): "전체적으로 디자인 더 세련되게 재디자인, 애플 것 최신 디자인 싹 다 찾아서
벤치마킹 / A, B, C 바꿀 때 내가 이 페이지를 바꾸고 있는지를 잘 모르겠어서 잠깐 깜빡이거나 이런
편의성 기능 / 갬성 있고 화면 레이아웃 비율도 애플과 유사하게 / 버튼이나 호버링 애니메이션도
애플처럼 통통 튀고 부드럽고 이음새가 어울리게."

### 벤치마킹의 전제 — 이미 우리가 애플 식을 쓰고 있다

이 저장소의 스프링은 **Apple `Spring(duration:bounce:)` 문서 공식과 정확히 일치**하고
`appleSpring.test.ts`가 애플 문서의 worked example로 고정한다(연구 문서 §1). 그러니 "싹 다"는
갈아엎는 것이 아니라 그 위에 쌓는 것이다.

**가져온 것** (2025 WWDC → iOS/macOS 26 Liquid Glass): 동심 반경, 떠 있는 레이어, 모프하는 피드백.
**가져오지 않은 것**: 도킹 패널의 판유리 투명 — **애플 자신이 26.1에서 `Tinted` 옵션으로 물러섰고**
(가독성), A-04a가 먼저 같은 결론에 도달해 있었다.

### M1 — 반경 사다리 하나, 그리고 동심

셸에 사다리가 **둘**(`s/m/l`과 `control/card/stage`) 있었고 raw 픽셀 반경이 **열한 개** 있었다.
사다리가 둘이면 없는 것과 같다. 이제:

```
stage 22 − inset 6 = card 16 − inset 6 = control 10 − tight 4 = chip 6
```

HIG의 중첩 규칙(안쪽 반경 = 바깥 반경 − 패딩) 그대로다. `radiusLadder.test.ts`가 산술과
"셸에 raw 픽셀 반경 금지"를 고정한다. `.simpleBar`의 세로 패딩을 11 → **12**로 올린 이유도 이것 —
22 − 12 = 10 = control이라 **계산되는** 동심 코너가 된다.

### M2 — 누름은 어디서나 같다

셸에 hover 규칙이 **18개**, press 규칙이 **3개**였다. 대부분의 컨트롤이 포인터가 오면 밝아지고
클릭하면 **완전히 가만히** 있었다. `.btn`이 들고 있던 레시피를 모든 누를 수 있는 표면에 폈다:
내려갈 때는 빠르고 평평하게(80ms — Miller 1968의 "내가 조작하는 기계의 일부" 상한 0.1s 안),
올라올 때는 누름 스프링(오버슈트가 물성으로 읽힌다).

bounce는 낮게 유지 — Kao(2020, N=3018): juice는 **역 U자**이고 대가는 취향이 아니라 **수행 성능**이다.
hover 리프트는 **뜨는 것에만**, `prefers-reduced-motion`이면 색만 남고 기하는 사라진다.

### M3 — 보드가 다른 보드가 되면 그렇게 말한다

A→B 전환은 피치의 모든 토큰을 갈아치우면서 **아무 말도 하지 않았다**. 이제 (a) 스테이지가 한 번
숨을 쉬고 (b) 어느 판인지 토스트가 말한다. 같은 신호를 **문서 정체성이 바뀌는 다른 자리**(새로 시작,
Shift+R)에도 쓴다.

조각이 아니라 **스테이지가** 움직인다 — Scholl & Pylyshyn(1999)이 이 피치에서 토큰이 절대
페이드/축소로 사라지면 안 되는 이유다(F(1,14)=51.02, p<.001).

**숨의 watermark는 ref가 아니라 모듈 스코프에 있다.** variant 전환은 보드 서브트리를 **교체**한다
(실측: 전환 후 스테이지 노드가 다른 노드). ref watermark는 알아채야 할 그 사건으로 초기화되고,
전환 전에 잡아 둔 ref는 버려질 노드를 가리킨다. 프로브가 `animate() calls: []`로 잡아냈다.

### M4 — 그림자는 "떠 있음"을 뜻한다

rest 그림자를 넓혔다가 **되돌렸다**(R-4): 사이드바 카드 4장이 저마다 그림자를 지면 컬럼이 더미가
되고, 무엇보다 **모두 떠 있으면 아무것도 떠 있지 않다**. rest는 헤어라인 + 가장 옅은 자리로 돌리고
raised 이상을 풍성하게 했다. 결과적으로 우상단 재생 패널은 확실히 뜨고, 도킹된 카드는 바닥에 앉는다.

채워진 컨트롤(기본 버튼·활성 칩·재생 버튼)에는 **위쪽 1px 하이라이트**를 넣었다. 색은 늘리지
않는다(VDR-0001 배제 3: 고채도 다색 금지) — 빛의 방향만 준다.

사이드바 카드 4장을 하나의 면으로 합치려던 안은 **폐기**(R-5): 카드 간 16px 간격은 근거 있는
결정이다(Kubovy Pure Distance Law — 12px이면 비율 1.5로 그룹이 다중안정).

### 문구

`Space — 처음부터 재생·일시정지`가 222px 패널에서 **단어 중간에 끊겼다**(스크린샷 실측).
`처음부터 재생·정지`로 줄임.

Files: src/ui/tokens.css, src/ui/shell.module.css, src/ui/StepBar.tsx, src/ui/AppShell.tsx,
src/ui/SidePanels.tsx, src/ui/useEditorKeyboard.ts, src/ui/keymap.ts, src/ui/i18n/ko.ts,
src/editor/uiStore.ts, src/ui/useBoardBreath.ts(신규), src/ui/radiusLadder.test.ts(신규),
pw/shell-feedback.cjs(신규), pw/audit-manifest.json, docs/agent/plans/ACTIVE_PLAN.md

Validation: typecheck/lint/**350 tests**/build/harness PASS. 브라우저 **176 checks ALL PASS**
(신규 9). feel-lab 두 테마: 대비 라이트 **206/206**, 다크 **205/206**(남은 1건은 팀색 위 등번호 —
기존 글리프 그림자로 완화), 토큰 어휘를 벗어난 모션 지속시간 **0/10**, 히트 타깃 2건은 기존
(버전 칩·튜토리얼 링크).

Related: PLAN-20260825-016, ADR-0006(D2/D8), VDR-0001, DESIGN_RESEARCH_2026-08-22

## CHG-20260825-212 — "시작 위치를 잡으면 왜 어쩔 땐 전체가 움직이지?" — 계약을 미터로 못 박고 안내를 고침

Date: 2026-08-25 · Type: DOC/UX · Level: L1

Problem (사용자 2026-08-25): "왜 어쩔 땐 0단계 레이어 잡으면 전체 단계가 움직이고 어쩔 땐 0단계만
움직여?"

답: **버그가 아니라 선택 개수로 갈리는 의도된 계약**이다(ADR-0009 v16b). 프로브로 실측:

| 선택 | 시작점 | 그 경로의 끝 | 이후 단계 |
| --- | --- | --- | --- |
| **1개** | 8.00m | **0.00m** | **0.00m** |
| **2개 이상** | 8.00m | **8.00m** | **8.00m** |

즉 한 명만 잡으면 **첫 다리만 늘어나고** 그 끝과 이후 단계는 얼어 있다. 여러 명이 선택돼 있으면
**체인 전체가 평행이동**한다.

두 동작 다 같은 사용자가 요청한 것이다 — 2026-08-20(CHG-065) "시작점만 붙어가 경로가 과도하게
꺾임" → 전체 이동, 2026-08-22(CHG-141) "미래 시점까지 전부 따라오는데 이유가 있나" → 단독 이동.
v16b가 **선택 크기**로 갈라 둘 다 살렸다.

문제는 규칙이 아니라 **드래그하는 순간 보이지 않는 상태에 의존한다는 것**, 그리고 안내가 그 차이를
말하지 않았다는 것이다. `드래그 — 옮기기 (여러 명이면 같이)`는 "선수들이 같이 온다"로 읽히고
**경로 이야기는 한 마디도 없었다**.

Change:
- 안내 문구 → `드래그 — 한 명 = 시작점만 · 여러 명 = 경로까지 통째로`.
- **`pw/home-drag` (신규 프로브)** — 두 경우를 미터로 고정한다. 미묘하고 회귀하기 쉬운 계약이라
  문장이 아니라 숫자로 잡아 둔다.

Files: src/ui/keymap.ts, pw/home-drag.cjs(신규), pw/audit-manifest.json
